...
const CustomSelect = ({ options, value, onChange, placeholder, icon: Icon }) => {
  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      <div onClick={() => setIsOpen(!isOpen)} className="w-full p-3 rounded-xl border border-gray-200 bg-white cursor-pointer flex justify-between items-center hover:border-blue-400 transition text-sm">
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-60 overflow-y-auto custom-scrollbar">
          {options.map((opt) => (
            <div key={opt.value} onClick={() => { onChange(opt.value); setIsOpen(false); }} className={`p-3 text-sm cursor-pointer hover:bg-blue-50 transition flex items-center justify-between ${String(value) === String(opt.value) ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'}`}>
              {opt.label}
...
const TeacherMultiSelect = ({ teachers, selectedIds, onChange }) => {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      <div onClick={() => setIsOpen(!isOpen)} className="w-full p-3 rounded-xl border border-gray-200 bg-white cursor-pointer min-h-[50px] flex flex-wrap gap-2 hover:border-blue-400 transition">
        {selectedIds.length === 0 && <span className="text-gray-400 text-sm pt-1">Wybierz nauczycieli...</span>}
        {selectedIds.map(id => {
          const teacher = teachers.find(t => t.id === id);
          return teacher ? (
            <span key={id} className="bg-blue-50 text-blue-700 px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1 border border-blue-100">
              {teacher.full_name} <X size={12} className="cursor-pointer hover:text-blue-900" onClick={(e) => { e.stopPropagation(); toggleTeacher(id); }}/>
            </span>
          ) : null;
        })}
      </div>
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-60 overflow-y-auto custom-scrollbar">
          {teachers.map(t => (
            <div key={t.id} onClick={() => toggleTeacher(t.id)} className={`p-3 text-sm hover:bg-gray-50 cursor-pointer flex justify-between items-center ${selectedIds.includes(t.id) ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700'}`}>
...
const TableMultiSelect = ({ options, value, onChange, absentMembers = [] }) => {
  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      <div className="w-full min-h-[32px] px-2 py-1 bg-white border border-gray-200 rounded-lg text-xs cursor-pointer flex flex-wrap gap-1 items-center hover:border-blue-300 transition" onClick={()=>setIsOpen(!isOpen)}>
        {selectedItems.length===0?<span className="text-gray-400 text-[10px] italic">Wybierz...</span>:selectedItems.map((i,x)=><span key={x} className="bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded text-[10px] border border-blue-100 whitespace-nowrap">{i}</span>)}
      </div>
      {isOpen && <div className="absolute z-[9999] left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto custom-scrollbar">{options.map(p=><div key={p.id} className={`px-3 py-1.5 text-xs cursor-pointer flex items-center justify-between transition ${absentMembers.includes(p.full_name)?'bg-gray-50 text-gray-400 cursor-not-allowed':'hover:bg-blue-50 text-gray-700'} ${selectedItems.includes(p.full_name)?'bg-blue-50 text-blue-700 font-medium':''}`} onClick={()=>t(p.full_name, absentMembers.includes(p.full_name))}><span className={absentMembers.includes(p.full_name)?'line-through':''}>{p.full_name}</span>{selectedItems.includes(p.full_name)&&!absentMembers.includes(p.full_name)&&<Check size={12}/>}</div>)}</div>}
    </div>
  );
};
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50/50 text-xs text-gray-500 uppercase">
                      ...
                      <th className="p-3 font-semibold min-w-[150px] text-blue-600">Temat lekcji</th> {/* PRZENIESIONO TUTAJ */}
                    ...
                    {groupedPrograms[monthKey].sort((a, b) => new Date(a.date) - new Date(b.date)).map(prog => {
                      ...
                      return (
                        <tr key={prog.id} className="hover:bg-white/60 transition relative">
                          ...
                          <td className="p-2"><input className="w-full bg-transparent border-b border-transparent hover:border-blue-300 focus:border-blue-500 text-xs p-1 outline-none font-semibold text-blue-700 placeholder-blue-200" placeholder="Temat..." defaultValue={prog.szkolka?.temat || ''} onBlur={e => updateField(prog.id, 'temat', e.target.value)} /></td>
                          ...
                          <td className="p-2"><input className="w-full bg-transparent border-b border-transparent hover:border-gray-300 focus:border-blue-500 text-xs p-1 outline-none" placeholder="Notatki..." defaultValue={prog.szkolka?.notatki || ''} onBlur={e => updateField(prog.id, 'notatki', e.target.value)} /></td>
                        </tr>
                      );
                    })}
                  </tbody>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Małe SchWro</h1>
      </div>

      {/* 1. GRAFIK */}
      <section className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 p-6">
        <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Grafik Nauczycieli</h2></div>
      ...
      <section className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Grupy Wiekowe</h2>
          <button onClick={() => { setGroupForm({ id: null, name: '', teacher_ids: [], room: '', age_range: '' }); setShowGroupModal(true); }} className="bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm px-5 py-2.5 rounded-xl font-medium hover:shadow-lg transition flex items-center gap-2"><Plus size={18}/> Dodaj grupę</button>
        </div>
              ...
              <div key={group.id} className="bg-white/60 backdrop-blur-sm border border-gray-200/50 rounded-2xl p-5 hover:shadow-lg transition">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => { setGroupForm(group); setShowGroupModal(true); }} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg"><FileText size={16}/></button>
                ...
                <div className="space-y-2 mb-4 text-sm text-gray-600">
                  <div className="flex items-center gap-2"><GraduationCap size={16} className="text-blue-500"/> <span className="font-medium">Nauczyciele:</span> {teacherNames || 'Brak'}</div>
                ...
                <div className="flex gap-2 border-t border-gray-100 pt-3 mt-2">
                  <button onClick={() => { setCurrentGroup(group); setShowGroupStudentsModal(true); }} className="flex-1 bg-blue-50 text-blue-700 text-xs font-bold py-2 rounded-xl hover:bg-blue-100 transition flex items-center justify-center gap-1"><Users size={14}/> Uczniowie ({studentCount})</button>
                  <button onClick={() => { setCurrentGroup(group); setShowMaterialsModal(true); }} className="flex-1 bg-purple-50 text-purple-700 text-xs font-bold py-2 rounded-xl hover:bg-purple-100 transition flex items-center justify-center gap-1"><BookOpen size={14}/> Materiały ({group.materials?.length || 0})</button>
                </div>
              </div>
            );
          })}
      ...
      <section className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Wszyscy Uczniowie ({filteredStudents.length})</h2>
          <div className="flex gap-3 items-center">
            ...
            <button onClick={() => { setGlobalStudentForm({ id: null, full_name: '', birth_year: '', parent_info: '', notes: '', group_id: null }); setShowGlobalStudentModal(true); }} className="bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm px-4 py-2.5 rounded-xl font-medium hover:shadow-lg transition flex items-center gap-2"><UserPlus size={18}/> Nowy uczeń</button>
          </div>
        </div>
              ...
              {filteredStudents.map((s) => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-blue-50/30 transition">
                    ...
                    <td className="p-4"><span className={`px-2 py-1 rounded-lg text-xs font-bold ${s.group_id ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>{gName}</span></td>
                    <td className="p-4 text-right flex justify-end gap-2">
                      <button onClick={() => openEditStudent(s)} className="text-blue-600 font-medium hover:underline">Edytuj</button>
      ...
      <section className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Nauczyciele ({teachers.length})</h2>
          <button onClick={() => { setTeacherForm({ id: null, full_name: '', role: 'Nauczyciel', email: '', phone: '' }); setShowTeacherModal(true); }} className="bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm px-5 py-2.5 rounded-xl font-medium hover:shadow-lg transition flex items-center gap-2"><Plus size={18}/> Dodaj nauczyciela</button>
        </div>
        <div className="bg-white/50 backdrop-blur-sm rounded-2xl border border-gray-200/50 overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead className="bg-gradient-to-r from-blue-50/80 to-purple-50/80 text-gray-700 font-bold border-b border-gray-200/50">
            ...
            <tbody className="divide-y divide-gray-200/50">
              {teachers.map(t => (
                <tr key={t.id} className="hover:bg-blue-50/30 transition">
                  ...
                  <td className="p-4 text-right flex justify-end gap-2">
                    <button onClick={() => { setTeacherForm(t); setShowTeacherModal(true); }} className="text-blue-600 font-medium">Edytuj</button>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-md p-6 border border-white/20">
            ...
            <div className="space-y-4">
              ...
              <button onClick={saveGroup} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold mt-4">Zapisz Grupę</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showGlobalStudentModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-md p-6 border border-white/20">
            <div className="flex justify-between mb-6"><h3 className="font-bold text-xl text-blue-700">{globalStudentForm.id ? 'Edytuj Ucznia' : 'Nowy Uczeń'}</h3><button onClick={() => setShowGlobalStudentModal(false)}><X/></button></div>
            <div className="space-y-4">
              ...
              <button onClick={saveGlobalStudent} className="w-full py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-bold mt-2">{globalStudentForm.id ? 'Zapisz zmiany' : 'Dodaj ucznia'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showGroupStudentsModal && currentGroup && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-3xl p-6 border border-white/20 flex flex-col max-h-[80vh]">
            <div className="flex justify-between mb-4 pb-4 border-b"><h3 className="font-bold text-xl text-blue-700">Uczniowie: {currentGroup.name}</h3><button onClick={() => setShowGroupStudentsModal(false)}><X/></button></div>
            <div className="bg-blue-50 p-4 rounded-xl mb-4 flex gap-3 items-end">
              <div className="flex-1">
                <label className="block text-xs font-bold text-blue-700 uppercase mb-1 ml-1">Przypnij ucznia z bazy</label>
              ...
              <button onClick={attachStudentToGroup} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-blue-700 h-[46px] flex items-center gap-2">Przypnij</button>
            </div>
      ...
      {showMaterialsModal && currentGroup && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-2xl p-6 border border-white/20 flex flex-col max-h-[80vh]">
            <div className="flex justify-between mb-4 pb-4 border-b"><h3 className="font-bold text-xl text-purple-700">Materiały: {currentGroup.name}</h3><button onClick={() => setShowMaterialsModal(false)}><X/></button></div>
            <div className="bg-purple-50 p-4 rounded-xl mb-4 space-y-2">
              ...
              <div className="flex gap-2 items-center">
                ...
                <button onClick={() => document.getElementById('file-upload').click()} className={`border px-4 rounded-xl flex items-center gap-2 h-[46px] transition ${materialForm.attachment ? 'bg-purple-100 border-purple-300 text-purple-700' : 'bg-white border-purple-200 text-gray-600 hover:bg-gray-50'}`}>
                  <Upload size={16}/> {materialForm.attachment ? 'Plik wybrany' : 'Wybierz plik'}
                </button>
                <button onClick={addMaterial} disabled={uploading} className="bg-purple-600 text-white px-6 rounded-xl font-bold hover:bg-purple-700 h-[46px] flex items-center gap-2">{uploading ? <Loader2 className="animate-spin"/> : 'Dodaj'}</button>
              </div>
            </div>
              ...
              {(currentGroup.materials || []).map(m => (
                <div key={m.id} className="flex items-center justify-between p-3 bg-white border rounded-xl hover:shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="bg-purple-100 p-2 rounded-lg text-purple-600"><BookOpen size={18}/></div>
                  ...
                  <div className="flex gap-2">
                    {m.attachmentUrl && <a href={m.attachmentUrl} target="_blank" rel="noreferrer" className="text-purple-600 hover:bg-purple-50 p-2 rounded-lg" title="Pobierz"><LinkIcon size={18}/></a>}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-md p-6 border border-white/20">
            ...
            <div className="space-y-4">
              ...
              <button onClick={saveTeacher} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold mt-4">Zapisz</button>
            </div>
          </div>
        </div>
      )}
