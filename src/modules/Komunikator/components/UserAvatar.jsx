...
export default function UserAvatar({ user, size = 'md', className = '', showStatus = false, status = null }) {
  ...
  const renderStatusIndicator = () => {
    ...
    return (
      <span
        className={`absolute ${statusSizeClasses[size]} ${statusColor} rounded-full ring-2 ring-white dark:ring-gray-800`}
        title={statusLabel}
      />
    );
  };
  ...
  return (
    <div className="relative inline-block">
      <div
        className={`${sizeClasses[size]} rounded-full flex items-center justify-center font-semibold text-white ring-2 ring-white dark:ring-gray-800 ${className}`}
        style={{ backgroundColor: bgColor }}
      >
        {initials || <User size={size === 'sm' ? 14 : size === 'lg' ? 20 : size === 'xl' ? 24 : 16} />}
      </div>
      {renderStatusIndicator()}
    </div>
  );
