  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      <div className="relative">
        <input
          className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 focus:ring-2 focus:ring-pink-500/20 outline-none placeholder:text-gray-400"
      ...
      {isOpen && (
        <div className="absolute z-[100] mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl max-h-60 overflow-y-auto custom-scrollbar">
          {options.map((opt) => (
            <div
              key={opt}
              className="px-4 py-2 hover:bg-pink-50 cursor-pointer text-sm text-gray-700 border-b border-gray-50 last:border-0"
              onClick={() => {
                onChange(opt);
                setIsOpen(false);
              }}
...
const MultiSelect = ({ label, options, value, onChange, absentMembers = [] }) => {
  ...
  return (
    <div ref={wrapperRef} className="relative group">
      ...
      <div 
        className="w-full min-h-[42px] px-4 py-2 bg-white/50 backdrop-blur-sm border border-gray-200/50 rounded-xl focus-within:ring-2 focus-within:ring-pink-500/20 cursor-pointer flex flex-wrap gap-2 items-center"
        onClick={() => setIsOpen(!isOpen)}
      >
        {selectedItems.length === 0 ? (
          ...
          selectedItems.map((item, idx) => (
            <span key={idx} className="bg-pink-100 text-pink-700 px-2 py-0.5 rounded-lg text-xs font-medium border border-pink-200 flex items-center gap-1">
              {item}
              <span 
...
const SongSelector = ({ songs, onSelect }) => {
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      <div 
        className="w-full px-3 py-2 bg-pink-50 border border-pink-100 rounded-lg text-sm text-pink-800 font-medium flex items-center justify-between cursor-pointer hover:bg-pink-100 transition"
        onClick={() => setIsOpen(!isOpen)}
      >
          ...
          <div className="overflow-y-auto flex-1 max-h-48 custom-scrollbar">
            {filteredSongs.length === 0 ? (
              ...
              filteredSongs.map(s => (
                <div 
                  key={s.id}
                  className="px-4 py-2 hover:bg-pink-50 cursor-pointer text-sm text-gray-700 flex justify-between items-center border-b border-gray-50 last:border-0"
                  onClick={() => {
                    onSelect(s);
                    setIsOpen(false);
                    setSearch('');
...
const SortableSongItem = ({ item, idx, songDef, onRemove, onChangeKey }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center justify-between gap-2 px-3 py-1.5 bg-white border border-pink-100 rounded-lg shadow-sm group">
      <div {...attributes} {...listeners} className="cursor-grab text-gray-300 hover:text-pink-600 active:cursor-grabbing">
        <GripVertical size={14} />
      </div>

      <div className="flex items-center gap-2 flex-1">
        <span className="text-pink-700 font-medium text-xs">{idx + 1}.</span>
...
const SortableRow = ({ row, index, program, setProgram, songs }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="grid grid-cols-12 gap-4 p-3 items-start hover:bg-pink-50/30 transition duration-150 bg-white border-b border-gray-100 last:border-0">
      <div className="col-span-1 flex items-center justify-center pt-2 cursor-grab text-gray-300 hover:text-pink-500 active:cursor-grabbing" {...attributes} {...listeners}>
  ...
  const ProgramItem = ({ p }) => (
    ...
    >
      <div className="flex justify-between items-start">
        <div>
          <div className={`font-bold text-sm mb-0.5 ${selectedId === p.id ? 'text-pink-700' : 'text-gray-700'}`}>
      ...
      <div className="absolute right-2 top-2 flex gap-1 opacity-0 group-hover:opacity-100 transition duration-200">
        <button onClick={(e) => { e.stopPropagation(); handleDuplicate(p); }} className="p-1 bg-pink-50 text-pink-600 rounded hover:bg-pink-100" title="Duplikuj"><Copy size={14} /></button>
    ...
    <div className="flex h-full bg-gradient-to-br from-pink-50 via-orange-50 to-pink-50">
      ...
      <div className="w-80 bg-white/40 backdrop-blur-xl border-r border-white/40 flex flex-col h-full">
        <div className="p-6 border-b border-white/40">
          ...
          <div className="flex gap-2 mb-3">
            <div className="relative flex-1">
              <input 
                placeholder="Szukaj..." 
                className="w-full px-4 py-2 bg-white/60 backdrop-blur-sm border border-gray-200/50 rounded-xl text-sm focus:ring-2 focus:ring-pink-500/20 outline-none" 
                value={filter} 
          ...
          <button onClick={() => setSelectedId(null)} className="w-full bg-gradient-to-r from-pink-600 to-orange-600 text-white py-2 rounded-xl font-bold shadow-lg hover:shadow-pink-500/30 transition transform hover:-translate-y-0.5 text-sm flex items-center justify-center gap-2"><Plus size={16} /> Nowy Program</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          ...
          <div>
            <button onClick={() => setShowHistory(!showHistory)} className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1 hover:text-pink-600 transition w-full text-left"><History size={12} /> Historia {showHistory ? <ChevronUp size={12} /> : <ChevronDown size={12} />}</button>
