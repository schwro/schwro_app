...
import { 
  ...
  LayoutGrid, List, CheckSquare, Filter, MessageSquare, Send, Clock
} from 'lucide-react';
...
export default function MediaTeamModule() {
  ...
  const addComment = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('media_task_comments').insert([commentData]);
      if (error) throw error;
  ...
  const toggleTaskCompletion = async (task) => {
    ...
    try {
      const { error } = await supabase.from('media_tasks').update({ status: newStatus }).eq('id', task.id);
      if (error) throw error;
    } catch (err) {
  ...
  // --- ZAPIS DANYCH ---
  const saveMember = async () => { /* ... (bez zmian) ... */ 
    try {
      if (!memberForm.full_name.trim()) { alert('Wymagane imię'); return; }
      if (memberForm.id) { await supabase.from('media_team').update(memberForm).eq('id', memberForm.id); }
      else { const { id, ...rest } = memberForm; await supabase.from('media_team').insert([rest]); }
      setShowMemberModal(false); fetchData();
    } catch (err) { alert(err.message); }
  };

  const deleteMember = async (id) => { /* ... (bez zmian) ... */
    if (confirm('Usunąć?')) { await supabase.from('media_team').delete().eq('id', id); fetchData(); }
  };

  const saveTask = async () => {
    try {
      if (!taskForm.title.trim()) { alert('Wymagany tytuł'); return; }
      const taskData = {
      ...
      if (taskForm.id) { await supabase.from('media_tasks').update(taskData).eq('id', taskForm.id); }
      else { await supabase.from('media_tasks').insert([taskData]); }
  ...
  const deleteTask = async (id) => {
    if (confirm('Usunąć zadanie?')) { await supabase.from('media_tasks').delete().eq('id', id); fetchData(); }
  };
  ...
  const handleDrop = async (e, newStatus) => {
    ...
    try { await supabase.from('media_tasks').update({ status: newStatus }).eq('id', draggedTask.id); fetchData(); } catch (err) {}
    setDraggedTask(null);
  };
              ...
              <div key={status} className={`bg-gradient-to-br from-gray-50/80 to-gray-100/50 backdrop-blur-sm rounded-2xl border-2 p-4 transition-all ${dragOverColumn === status ? 'border-blue-400 bg-blue-50/50 shadow-lg' : 'border-gray-200/50'}`}
                ...
                <div className="space-y-3">
                  {filteredTasks.filter(t => t.status === status).map(task => {
                    const isOverdue = task.due_date && new Date(task.due_date) < new Date().setHours(0,0,0,0) && task.status !== 'Gotowe';
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between items-start mb-2">
                              <h4 className={`font-bold text-gray-800 ${task.status === 'Gotowe' ? 'line-through text-gray-500' : ''}`}>{task.title}</h4>
                              <GripVertical size={16} className="text-gray-300 opacity-0 group-hover:opacity-100 transition" />
                            </div>
                            
                            {/* DATA WYKONANIA W KAFELKU */}
                            <div className="flex flex-wrap gap-2 mb-3">
                              {task.due_date && (
                                <div className={`flex items-center gap-1 text-xs px-2 py-1 rounded-md font-medium border ${
                                  isOverdue 
                              ...
                              {task.assigned_to && (
                                <div className="flex items-center gap-1 text-xs px-2 py-1 rounded-md font-medium bg-blue-50 text-blue-600 border border-blue-100">
                                  <User size={12} />
                                  {team.find(m => m.id === task.assigned_to)?.full_name.split(' ')[0]}
                                </div>
                              )}
                            </div>
                            ...
                            <div className="flex justify-between items-center border-t border-gray-100 pt-2">
                              <div className="text-xs text-gray-400 flex items-center gap-1">
                                {task.attachment && <Paperclip size={12} />}
                              </div>
                              <button onClick={() => openTaskModal(task)} className="text-blue-600 text-xs font-medium hover:underline">Szczegóły</button>
                            </div>
                          </div>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-4xl p-0 border border-white/20 my-8 flex overflow-hidden h-[80vh]">
            ...
            <div className="w-3/5 p-8 overflow-y-auto border-r border-gray-200/50">
              <div className="flex justify-between mb-6"><h3 className="font-bold text-2xl bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">{taskForm.id ? 'Edycja zadania' : 'Nowe zadanie'}</h3></div>
              <div className="space-y-5">
                <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tytuł</label><input className="w-full px-4 py-3 border border-gray-200/50 rounded-xl bg-white/50" value={taskForm.title} onChange={e => setTaskForm({...taskForm, title: e.target.value})} /></div>
                <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Opis</label><textarea className="w-full px-4 py-3 border border-gray-200/50 rounded-xl bg-white/50 h-32" value={taskForm.description} onChange={e => setTaskForm({...taskForm, description: e.target.value})} /></div>
                ...
                <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Przypisana osoba</label><select className="w-full px-4 py-3 border border-gray-200/50 rounded-xl bg-white/50" value={taskForm.assigned_to || ''} onChange={e => setTaskForm({...taskForm, assigned_to: e.target.value ? parseInt(e.target.value) : null})}><option value="">Nie przypisano</option>{team.map(m => <option key={m.id} value={m.id}>{m.full_name}</option>)}</select></div>
                <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Załącznik</label><input type="file" className="w-full text-sm" onChange={handleFileUpload} />{taskForm.attachment && <div className="mt-2 text-sm text-gray-600">{taskForm.attachment.name} <button onClick={() => setTaskForm({...taskForm, attachment: null})} className="text-red-500 ml-2">Usuń</button></div>}</div>
                <div className="pt-6 flex justify-end gap-3">
                  {taskForm.id && <button onClick={() => { deleteTask(taskForm.id); setShowTaskModal(false); }} className="text-red-500 px-4 py-2">Usuń</button>}
                  <button onClick={saveTask} className="px-6 py-3 bg-blue-600 text-white rounded-xl">Zapisz</button>
                </div>
              </div>
            </div>

            <div className="w-2/5 bg-gray-50/50 p-6 flex flex-col">
              <div className="flex justify-between items-center mb-4"><h4 className="font-bold text-gray-700">Komentarze</h4><button onClick={() => setShowTaskModal(false)}><X size={20}/></button></div>
              <div className="flex-1 overflow-y-auto space-y-4 mb-4">
                {!taskForm.id ? <div className="text-center text-gray-400 mt-10">Zapisz zadanie, aby komentować.</div> : comments.map(c => (
                  <div key={c.id} className="bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
                    <div className="flex justify-between mb-1"><span className="font-bold text-xs text-blue-700">{c.author_name}</span><span className="text-[10px] text-gray-400">{new Date(c.created_at).toLocaleString()}</span></div>
                    <p className="text-sm">{c.content}</p>
                  </div>
                ))}
              </div>
              {taskForm.id && <div className="relative"><textarea className="w-full pl-4 pr-12 py-3 border rounded-xl bg-white resize-none" rows={2} value={newComment} onChange={e => setNewComment(e.target.value)} onKeyDown={e => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); addComment(); }}} placeholder="Napisz komentarz..." /><button onClick={addComment} className="absolute right-2 bottom-2 p-2 bg-blue-600 text-white rounded-lg"><Send size={16}/></button></div>}
            </div>
      ...
      {showMemberModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-md p-6 border border-white/20">
            <div className="flex justify-between mb-6"><h3 className="font-bold text-xl">Członek zespołu</h3><button onClick={() => setShowMemberModal(false)}><X size={20}/></button></div>
            <div className="space-y-4">
              <input className="w-full px-4 py-3 border rounded-xl" placeholder="Imię i nazwisko" value={memberForm.full_name} onChange={e => setMemberForm({...memberForm, full_name: e.target.value})} />
              <input className="w-full px-4 py-3 border rounded-xl" placeholder="Rola" value={memberForm.role} onChange={e => setMemberForm({...memberForm, role: e.target.value})} />
              <input className="w-full px-4 py-3 border rounded-xl" placeholder="Email" value={memberForm.email} onChange={e => setMemberForm({...memberForm, email: e.target.value})} />
              <input className="w-full px-4 py-3 border rounded-xl" placeholder="Telefon" value={memberForm.phone} onChange={e => setMemberForm({...memberForm, phone: e.target.value})} />
              <div className="flex justify-end gap-3"><button onClick={() => setShowMemberModal(false)} className="px-5 py-2.5 border rounded-xl">Anuluj</button><button onClick={saveMember} className="px-5 py-2.5 bg-blue-600 text-white rounded-xl">Zapisz</button></div>
            </div>
          </div>
        </div>
      )}
