...
function SongDetailsModal({ song, onClose, onEdit }) {
  ...
  const generatePDF = () => {
    import('jspdf').then(({ jsPDF }) => {
      ...
      doc.text(song.title || 'Bez tytułu', margin, yPos);
      yPos += 12;
      ...
      doc.text(
        `Tonacja: ${displayKey} (oryginalna: ${originalKey}) | Tempo: ${song.tempo ? song.tempo + ' BPM' : '–'} | Metrum: ${song.meter || '–'}`,
        margin,
        yPos
      );
      yPos += 10;
      ...
      doc.setFontSize(9);
      doc.setFont(undefined, 'bold');
      doc.text('Treść', leftColX, yPos);
      doc.text('Akordy w taktach', rightColX, yPos);
      yPos += 6;
      ...
      for (let i = 0; i < maxLines; i++) {
        ...
        doc.text(lyricLine, leftColX, yPos, { maxWidth: colWidth - 2 });
        doc.text(chordLine, rightColX, yPos, { maxWidth: colWidth - 2 });
